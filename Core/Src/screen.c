...
void Screen_Init(void) // this function is called in main
{
	...
	float db_values[] = { 0, -5, -10 }; // dB labels that get plotted
	for (int i = 0; i < 3; i++) // plotting dB labels
		...
		lv_obj_set_style_text_color(label, lv_color_hex(0x808080),
				LV_PART_MAIN);
		int y = db_to_y(db_values[i], height);
		lv_obj_set_pos(label, -12, y - 20);
	}
...
void Screen_Add_Sample(void) {
	if (fft_ready || !fft_init)
		return;
	int16_t val = dac_buf[0];
	realbuf[fft_index++] = val;
...
void Compute_FFT(void) {
	...
	arm_rfft_fast_f32(&fft_instance, realbuf, magnitude, 0);
	fft_ready = 0;
	arm_cmplx_mag_f32(magnitude, magnitude, FFT_SIZE/2);

	double freq_per_bin = SAMPLE_RATE / FFT_SIZE;
	double log_freq_max = log10(20000);
	double log_freq_min = log10(20);
	double log_freq_per_band = (log_freq_max - log_freq_min) / NUM_BANDS;

	for (int band = 0; band < NUM_BANDS; band++) {
		double min_freq_in_band = pow(10.0,
				log_freq_per_band * band + log_freq_min);
		double max_freq_in_band = pow(10.0,
				log_freq_per_band * (band + 1) + log_freq_min);
		...
		double avg = sum / (max_bin - min_bin); //pow(, 1);
		band_values[band] = 20.0f * log10f(avg + 1e-6f) - 60.0;
	}
}
...
void Screen_Update(void) {
	...
	float max_value = 25;
	for (int i = 0; i < NUM_BANDS; i++) {
		if (band_values[i] > max_value) {
			max_value = band_values[i];
		}
	}
	running_max = max_value * 0.1 + running_max * 0.9;
	if (running_max > max_value)
		max_value = running_max;
	float scale = 150.0 / pow(5, (max_value) / 10);

	for (int i = 0; i < NUM_BANDS; i++) {
		float db = band_values[i];

		int bar_height = pow(5, db / 10) * scale + 10;
		if (bar_height < 10)
			bar_height = 10;
		if (bar_height > 200)
			bar_height = 200;
